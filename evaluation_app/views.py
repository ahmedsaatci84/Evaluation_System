...
def database_backup(request):
    ...
    try:
        ...
        if backup_dir.exists():
            # Support both .bak (native) and .json (Django dumpdata) backups
            backup_files = list(backup_dir.glob('*.bak')) + list(backup_dir.glob('*.json'))
            for backup_file in sorted(backup_files, key=lambda x: x.stat().st_mtime, reverse=True):
                try:
                    file_stat = backup_file.stat()
                    backup_type = 'Native DB' if backup_file.suffix == '.bak' else 'JSON Data'
                    backups.append({
...
def database_backup_create(request):
    ...
    try:
        ...
        backup_method = "native"  # Track which backup method was used
        ...
        if db_engine == 'mssql':
            ...
            try:
                ...
                with connection.cursor() as cursor:
                    # First, verify we have backup permissions
                    cursor.execute("SELECT HAS_PERMS_BY_NAME(NULL, NULL, 'BACKUP DATABASE')")
                    has_perm = cursor.fetchone()[0]
                    
                    if not has_perm:
                        # Fallback to Django dumpdata if no SQL Server backup permission
                        logger.warning("No BACKUP DATABASE permission, using Django dumpdata fallback")
                    ...
                    cursor.execute(f"""
                    RESTORE VERIFYONLY FROM DISK = N'{backup_path_str}'
                    """)
                    
            except PermissionError:
                # Use Django's dumpdata as fallback
                ...
                logger.info("Using Django dumpdata fallback method")
                
                # Use Django's dumpdata command without --output flag to get stdout
                # This avoids encoding issues with file writing
                ...
                if result.returncode != 0:
                    ...
                    raise Exception(f"Django dumpdata failed: {error_msg}")
                
                # Write the output to file with explicit UTF-8 encoding
                try:
                    # Decode the output as UTF-8
                    json_data = result.stdout.decode('utf-8', errors='replace')
        ...
        logger.info(f"Backup completed: {backup_filename} ({format_file_size(file_size)}) using {backup_method}")
        
        # Create appropriate success message based on backup method
        if backup_method == "django_dumpdata":
            success_msg = f'Database backup created (JSON format - data only). File: {backup_filename}'
        else:
    ...
    try:
        ...
        if db_engine == 'mssql':
            ...
            with connection.cursor() as cursor:
                ...
                if not has_perm:
                    raise PermissionError("User does not have RESTORE DATABASE permission")
                ...
                try:
                    ...
                    # Always return to multi-user mode
                    cursor.execute(f"""
